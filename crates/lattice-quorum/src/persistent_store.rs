...
pub struct FileLogStore {
    inner: Arc<RwLock<FileLogStoreInner>>,
}
...
impl FileLogStore {
    ...
    fn load_wal(wal_dir: &Path) -> io::Result<(BTreeMap<u64, EntryOf>, Option<LogIdOf>)> {
        let mut log = BTreeMap::new();
        let mut last_purged: Option<LogIdOf> = None;
        ...
        for entry_result in std::fs::read_dir(wal_dir)? {
            ...
            match serde_json::from_str::<EntryOf>(&data) {
                Ok(log_entry) => {
...
mod tests {
    ...
    async fn initial_state_is_empty() {
        ...
        let mut store = FileLogStore::new(dir.path()).unwrap();
    ...
    async fn save_and_read_vote() {
        ...
        let mut store2 = FileLogStore::new(dir.path()).unwrap();
    ...
    async fn wal_directory_created() {
        let dir = tempfile::tempdir().unwrap();
        let _store = FileLogStore::new(dir.path()).unwrap();
