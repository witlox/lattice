...
use lattice_common::types::{
    ...
    NodeId, NodeOwnership, NodeState, RegisteredEndpoint, ServiceRegistryEntry, Tenant, TenantId,
    TenantQuota, TopologyModel, VCluster, VClusterId,
};
...
impl GlobalState {
    ...
    pub fn apply(&mut self, cmd: Command) -> CommandResponse {
        let resp = match cmd {
            ...
            Command::RequeueAllocation { id } => self.requeue_allocation(id),
    ...
    fn requeue_allocation(&mut self, id: AllocId) -> CommandResponse {
        let Some(alloc) = self.allocations.get_mut(&id) else {
            return CommandResponse::Error(format!("Allocation not found: {id}"));
        };
...
mod tests {
    ...
    fn endpoints_deregistered_on_requeue() {
        ...
        // Requeue
        state.apply(Command::RequeueAllocation { id: alloc_id });
